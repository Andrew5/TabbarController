...
- (id)initWithItems:(NSArray *)items {
    self = [super init];
    if (self) {
        self.frame = CGRectMake(0, [UIScreen mainScreen].bounds.size.height - HEIGHT_MENU_VIEW, [UIScreen mainScreen].bounds.size.width, 45);
...
- (void)setupButtons {
    NSInteger count = 0;
    NSInteger xExtra = 0;
    NSInteger buttonSize = floor([UIScreen mainScreen].bounds.size.width / [self.buttonData count]) -1;
        ...
        if ([self.buttonData count] % 2 == 1) {
            if ([self.buttonData count] == 5) {
                ...
                if (i == [self.buttonData count]) {
                    ...
                    buttonSize = floor([UIScreen mainScreen].bounds.size.width / [self.buttonData count]);
                }
            }else{
                if (count + 1 == 2) {
                    extra = 1;
